...
Synced from `.claude/skills/hotfix/SKILL.md`.

## Codex Notes

- Invoke this workflow with `$hotfix`.
...
6b. **Collect approvals** — Request sign-off explicitly in conversation or from Codex subagents when parallel review is warranted:
   - load `.codex/references/agents/lead-programmer.md` — Review the fix for correctness and side effects
   - load `.codex/references/agents/qa-tester.md` — Run targeted regression tests on the affected system
   - load `.codex/references/agents/producer.md` — Approve deployment timing and communication plan
