...
Synced from `.claude/skills/design-system/SKILL.md`.

## Codex Notes

- Invoke this workflow with `$design-system`.
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   plain-text option prompts for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use plain-text option prompts to capture the decision.
...
**Agent delegation**: For complex mechanics, use the subagent tools to delegate to
...
delegate to `systems-designer` via the subagent tools. Provide:
...
**When delegating via subagent tools**:
...
- The main session presents the agent's output to the user via plain-text option prompts
...
2. **plain-text option prompts** at every decision point (Explain -> Capture pattern):
