...
Use this file as the role prompt when you spawn a Codex subagent for `godot-specialist`.
...
You may delegate to Codex subagents using the matching role profiles when the user explicitly asked for team or parallel work. Use it when a task requires deep expertise in a specific Godot subsystem:

- load `.codex/references/agents/godot-gdscript-specialist.md` — GDScript architecture, static typing, signals, coroutines
- load `.codex/references/agents/godot-shader-specialist.md` — Godot shading language, visual shaders, particles
- load `.codex/references/agents/godot-gdextension-specialist.md` — C++/Rust bindings, native performance, custom nodes
